...
public class TypeToken<T> {
  ...
  protected TypeToken() {
    ...
    this.rawType = (Class<? super T>) GsonTypeChecker.getRawType(type);
  ...
  private TypeToken(Type type) {
    this.type = GsonTypeChecker.canonicalize(Objects.requireNonNull(type));
    this.rawType = (Class<? super T>) GsonTypeChecker.getRawType(this.type);
   ...
   * type argument for {@code T} in {@link GsonTypeChecker#canonicalize canonical form}.
   */
  private Type getTypeTokenTypeArgument() {
    ...
    if (superclass instanceof ParameterizedType) {
      ...
      if (parameterized.getRawType() == TypeToken.class) {
        Type typeArgument = GsonTypeChecker.canonicalize(parameterized.getActualTypeArguments()[0]);
  ...
  public boolean isAssignableFrom(Type from) {
    ...
    if (type instanceof Class<?>) {
      return rawType.isAssignableFrom(GsonTypeChecker.getRawType(from));
    } else if (type instanceof ParameterizedType) {
  ...
  private static boolean isAssignableFrom(
    ...
    Class<?> clazz = GsonTypeChecker.getRawType(from);
  ...
  public final boolean equals(Object o) {
    return o instanceof TypeToken<?> && GsonTypeChecker.equals(type, ((TypeToken<?>) o).type);
  }

  @Override
  public final String toString() {
    return GsonTypeChecker.typeToString(type);
  }
  ...
  public static TypeToken<?> getParameterized(Type rawType, Type... typeArguments) {
    ...
    if (GsonTypeChecker.requiresOwnerType(rawType)) {
      throw new IllegalArgumentException(
          "Raw type "
              + rawClass.getName()
    ...
    for (int i = 0; i < expectedArgsCount; i++) {
      ...
      Class<?> rawTypeArgument = GsonTypeChecker.getRawType(typeArgument);
      TypeVariable<?> typeVariable = typeVariables[i];

      for (Type bound : typeVariable.getBounds()) {
        Class<?> rawBound = GsonTypeChecker.getRawType(bound);
    ...
    return new TypeToken<>(GsonTypeChecker.newParameterizedTypeWithOwner(null, rawType, typeArguments));
  }
  ...
  public static TypeToken<?> getArray(Type componentType) {
    return new TypeToken<>(GsonTypeChecker.arrayOf(componentType));
  }
}
